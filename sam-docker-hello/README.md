...
sam init --name sam-docker-hello --package-type Image --base-image amazon/nodejs14.x-base
...
```bash
sam-docker-hello$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sam-docker-hello$ sam local start-api
sam-docker-hello$ curl http://localhost:3000/
```
...
```bash
sam-docker-hello$ sam logs -n HelloWorldFunction --stack-name sam-docker-hello --tail
```
...
```bash
sam-docker-hello$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name sam-docker-hello
```
